...
def _extract_ticker_from_query(query: str) -> str:
    if m := re.search(r"\$([A-Za-z]{1,5})\b", query):
        return m.group(1).upper()
    upp = re.findall(r"\b([A-Z]{2,5})\b", query)
    if upp:
        return upp[0]
    ticker_map = {
...
def generate_sql_with_context(query: str, ticker: str, dates: List[str], is_range: bool, table_info: str) -> str:
    intent = _analyze_query_intent(query)
    _dbg("QUERY-INTENT", intent)
    system_prompt = f"""
You are a SQL expert for stock price data ONLY. Generate a SELECT query that **must** use the table name exactly `stock_data` (do not change it).
...
QUERY ANALYSIS:
- User question: {query}
- Detected ticker: {ticker or "Must be provided"}
- Detected dates: {dates or "None - use recent data"}
- Is date range: {is_range}
- Intent: {intent.get('intent', 'general query')}
- Needed columns: {intent.get('columns', ['date', 'open', 'close'])}

STRICT REQUIREMENTS:
1. ONLY query the stock_data table - NEVER mention news, headlines, or articles
2. ALWAYS include 'ticker' and 'date' columns in SELECT
3. ALWAYS include WHERE ticker = '{ticker}' (if ticker provided)
4. For date ranges: WHERE date BETWEEN 'start_date' AND 'end_date'
5. For recent data: ORDER BY date DESC LIMIT 10
6. Use proper date format: 'YYYY-MM-DD'
7. Select only relevant columns based on user request
...
- Include ticker in WHERE clause if available
- Order by date ASC for ranges, DESC for recent data
- Limit results appropriately (10-50 rows max)
...
"""
    try:
        ...
        cleaned_sql = _force_fix_table_name(cleaned_sql)
        cleaned_sql = cleaned_sql.strip().rstrip(";")

        _dbg("SQL-GENERATION-CLEANED", cleaned_sql)
        return cleaned_sql
    except Exception as e:
        _dbg("SQL-GENERATION-ERROR", f"Failed to generate SQL: {e}")
        if ticker and dates and len(dates) >= 2:
            return f"SELECT ticker, date, open, close FROM stock_data WHERE ticker = '{ticker}' AND date BETWEEN '{dates[0]}' AND '{dates[1]}' ORDER BY date ASC"
        elif ticker:
            return f"SELECT ticker, date, open, close FROM stock_data WHERE ticker = '{ticker}' ORDER BY date DESC LIMIT 10"
        else:
            return "SELECT ticker, date, close FROM stock_data ORDER BY date DESC LIMIT 5"
...
def run_sql_agent(state: AgentState) -> AgentState:
    ...
    ticker = _extract_ticker_from_query(question)
    ...
    try:
        sql_query = generate_sql_with_context(question, ticker, dates, is_range, table_info)
